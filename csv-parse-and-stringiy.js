    ...
    for(let i=0; i<first_split.length; i++) {
      ...
      for(let j=0; j<rtnArray[i].length; j++) { // Let's check and modify each cells.
        ...
        if(!marge_status.marge && cell_value[0]==="\"") { // Meaning the start of a Wonderful-Cell.
          ...
          rtnArray[i][j] = cell_value.slice(1).replace(/""/g,"\""); // <- Remove \" from the head of the value and replace \"\" to \".
        }
        if(marge_status.marge) { // Meaning [in / the end of] a Wonderful-Cell.
          ...
          cell_value = (marge_status.row===i ? ",":"\r\n") + cell_value.replace(/""/g,"\""); // Restorate separator and replace \"\" to \".
          rtnArray[marge_status.row][marge_status.colmun] += cell_value; // <- Now, marge and...
  ...
  static stringify(array, option) {
    for(let i=0; i<array.length; i++) {
      for(let j=0; j<array[i].length; j++) {
        ...
        if(includesNewLine || includesComma || includesQuote) array[i][j] = `"${cell_value}"`;
      }
      array[i] = array[i].join(",");
    }
    return array.join(option.rowSeparator || "\r\n");
  }
};
